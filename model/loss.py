...
class EnhancedFrequencyLoss(nn.Module):
    def __init__(self, alpha=0.5, beta=0.2):
        super().__init__()
        self.alpha = alpha  # 幅度谱对齐权重
        self.beta = beta  # 高频细节保留权重
        self.gauss_kernel = self._get_gaussian_kernel(3, 1.5).cuda()

    @staticmethod
    def _get_gaussian_kernel(size=3, sigma=1.5):
        """生成可分离高斯核"""
        kernel_1d = torch.linspace(-(size // 2), size // 2, steps=size)
    ...
    def forward(self, fused, src1, src2):
        """
        简化后的前向计算
        :param fused: 融合图像 [B,1,H,W]
        :param src1: 源图像1 [B,1,H,W]
        :param src2: 源图像2 [B,1,H,W]
        """
        # 幅度谱对齐损失
        fused_amp, _ = self._get_spectrum(fused)
        src1_amp, _ = self._get_spectrum(src1)
        src2_amp, _ = self._get_spectrum(src2)
        amp_loss = F.l1_loss(fused_amp, torch.max(src1_amp, src2_amp))

        # 高频细节保留（使用高斯差分）
        def _high_pass(img):
            low = F.conv2d(img, self.gauss_kernel[None, None, ...], padding=1)
            return img - low

        hf_loss = F.mse_loss(
            _high_pass(fused),
            torch.max(_high_pass(src1), _high_pass(src2))
        )

        return {
            "total": self.alpha * amp_loss + self.beta * hf_loss,
            "amp": amp_loss,
            "hf": hf_loss
        }
