...
for i in tqdm(range(len(IR_files))):
    ...
    try:
        I_VIS = (
            imread(VIS_files[i]).astype(np.float32).transpose(2, 0, 1) / 255.0
        )  # [3, H, W] Uint8->float32
        I_VIS = rgb2y(I_VIS)  # [1, H, W] Float32
        I_IR = (
            imread(IR_files[i]).astype(np.float32)[None, :, :] / 255.0
        )  # [1, H, W] Float32
    except Exception as e:
        print(f"Error processing file {VIS_files[i]}: {e}")
        continue
    # crop
    ...
    for ii in range(I_IR_Patch_Group.shape[-1]):
        ...
        if not (bad_IR or bad_VIS):
            ...
            h5_vis.create_dataset(
                str(train_num), data=avl_VIS, dtype=avl_VIS.dtype, shape=avl_VIS.shape
            )
            train_num += 1
